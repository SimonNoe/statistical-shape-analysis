function [registered,targetV,targetF]=nonrigidICPv2(targetV,sourceV,targetF,sourceF,iterations,flag_prealligndata)
...
[error1,sourceV,transform]=rigidICP1(targetV,sourceV,1,Indices_edgesS,Indices_edgesT);
else
    [error1,sourceV,transform]=rigidICP1(targetV,sourceV,0,Indices_edgesS,Indices_edgesT);
end
...
for i =1:iterations
    ...
    [idx,d]=knnsearch(seedingmatrix,sourceV);
    tempidx=unique(idx);
...
%    
   ...
   %gaussian
    ...
    modes=ppi*reshape(vectors,3*r,1);
     ...
     D2 = pdist2(sourceV,seedingmatrix);
